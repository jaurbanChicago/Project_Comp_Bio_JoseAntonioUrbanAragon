...
Much population genetic theory concentrates on describing the changes of allele frequencies through time. The two most important factors influencing allele trajectories through time are natural selection and **genetic drift** [1]. In these vignettes, we will focus on genetic drift and how this process may be modeled as a [stochastic process](https://en.wikipedia.org/wiki/Stochastic_process) by using an idealized scenario called the [Wright-Fisher Model](https://stephens999.github.io/fiveMinuteStats/wright_fisher_model.html).    
...
In this section of the vignette, we will introduce three basic models that can be used to model properties of genetic drift:
  
  * Binomial distribution  
  * Markov chain  
  * Diffusion process (continuous time approximation to the Markov chain)  

### Binomial Distribution in Genetic Drift  
